...
#define IPC_Flag (u64)0x80
#define WRAP_Flag (u64)0x40
...
#define ARG0_MASK      0x000000000000ffffll
#define RETVAL_MASK    0x0000000000ff0000ll
#define DELTA_MASK     0x00000000ff000000ll
#define EVENT_MASK     0x00000fff00000000ll
#define TIMESTAMP_MASK 0xfffff00000000000ll
#define EVENT_DELTA_RETVAL_MASK (EVENT_MASK | DELTA_MASK | RETVAL_MASK)
#define EVENT_RETURN_BIT           0x0000020000000000ll
#define EVENT_LENGTH_FIELD_MASK 0x000000000000000fll

#define UNSHIFTED_RETVAL_MASK 0x00000000000000ffll
#define UNSHIFTED_DELTA_MASK  0x00000000000000ffll
#define UNSHIFTED_EVENT_MASK  0x0000000000000fffll
#define UNSHIFTED_TIMESTAMP_MASK 0x00000000000fffffll
#define UNSHIFTED_EVENT_RETURN_BIT 0x0000000000000200ll
#define UNSHIFTED_EVENT_HAS_RETURN_MASK 0x0000000000000c00ll
...
#define FULL_TIMESTAMP_MASK 0x00ffffffffffffffll
#define CPU_NUMBER_SHIFT 56

#define GETTIMEOFDAY_MASK 0x00ffffffffffffffll
...
inline u64 get_inst_retired(void) {
	u32 cycles;
	// Get current counted events (register PMXEVCNTR)
	asm volatile("mrc p15, 0, %0, c9, c13, 2" : "=r"(cycles));
	return cycles;
}
inline void setup_get_inst_retired(void) {
	// Select performance counter #8 (PMSELR register)
	asm volatile("mcr p15, 0, %0, c9, c12, 5" :: "r"(0x1));
	// Count instructions architecturally executed in all rings (PMXEVTYPER register)
	asm volatile("mcr p15, 0, %0, c9, c13, 1" :: "r"(0x8));
	// Enable cycle counter and counter #1 (PMNCNTENSET register)
	asm volatile("mcr p15, 0, %0, c9, c12, 1" :: "r"(0xf0000001));
}
static void enable_usermode_cyclecount(void* dummy) {
	u32 temp;
	// Enable user mode cycle counter access (PMUSERENR register)
	asm volatile("mcr p15, 0, %0, c9, c14, 0" :: "r"(0x1));
	// Disable interrupts on overflow for all 8 event counters (PMINTENCLR register)
	asm volatile("mcr p15, 0, %0, c9, c14, 2" :: "r"(0x800000ff));
	// Select cycle counter (PMSELR register)
	asm volatile("mcr p15, 0, %0, c9, c12, 5" :: "r"(0x1f));
	// Count cycles in all rings (PMXEVTYPER register)
	asm volatile("mcr p15, 0, %0, c9, c13, 1" :: "r"(0x8));
	// Enable cycle counter (PMNCNTENSET register)
	asm volatile("mcr p15, 0, %0, c9, c12, 1" :: "r"(0xf0000000));
	// Enable performance counters and cycle divisor (PMCR register)
	// Also reset counters to 0
	asm volatile("mrc p15, 0, %0, c9, c12, 0" : "=r"(temp));
	temp |= 0xf;
	asm volatile("mcr p15, 0, %0, c9, c12, 0" :: "r"(temp));
}/*
static void reset_counters(void* dummy) {
	// Reset performance counters and cycle divisor (PMCR register)
	uint32_t temp;
	asm volatile("mrc p15, 0, %0, c9, c12, 0" : "=r"(temp));
	temp |= 0x6;
	asm volatile("mcr p15, 0, %0, c9, c12, 0" :: "r"(temp));
...
printk(KERN_INFO "kutrace_ipc_mod rdMSR(0x38D) = %016llx\n", inst_ret_ctrl);
	inst_ret_ctrl &= ~0x000000000000000Fllu;
	inst_ret_ctrl |=  0x0000000000000003llu;
	wrMSR(0x38D, inst_ret_ctrl);

	/* Enable fixed inst_ret counter in IA32_PERF_GLOBAL_CTRL 0x38F */
	/* msr[ 0x38F] |= 0x0000000200000000llu; */
...
/* Arm is every 64th cycle. This is 16Mhz on a 1Ghz processor */
/* Arm64 returns 32MHz counts: 31.25 ns each */
/* Arm64 could by more exact by using the PMU cycle counter */
/* x86-64 version returns rdtsc() >> 6 to give ~20ns resolution */
/* In both cases, ku_get_cycles returns cycles/64, not cycles */
/*#if defined(__arm__)
inline u32 ku_get_cycles(void)
{
	u32 timer_value;
	asm volatile("mrc p15, 0, %0, c9, c13, 0" : "=r"(timer_value));
	return timer_value;
}
#else*/
inline u64 ku_get_cycles(void)
{
	volatile u64 timer_value;
#if defined(__arm__)
	const void __iomem *TIMER_BASE = ioremap(0x00A00200, 8);
	volatile u32 upper_32;
	volatile u32 lower_32;
	// Avoid measuring during a rollover
	// See 4.4.1 in the Cortex-A9 MPCore Technical Reference Manual
	do {
		upper_32 = readl_relaxed(TIMER_BASE + 0x4);
		lower_32 = readl_relaxed(TIMER_BASE);
	} while (upper_32 != readl_relaxed(TIMER_BASE + 0x4));
	timer_value = upper_32;
	timer_value <<= 32;
	timer_value |= lower_32;
	// The global timer counts at 1/2 the core clock, so 500Mhz
	// Cut that down to 62Mhz; ~16ns resolution
	timer_value >>= 3;
#elif defined(__aarch64__)
 ...
 */
/* Tracing must be off and flush must have been called */
/* If subscr is < 0, we only return the top 32 bits */
static u64 get_word(s64 subscr)
{
	...
	return (u32)(blockp[u64_within_block] >> shift);
}
...
/* If subscr is < 0, we only return the top 32 bits */
static u64 get_ipc_word(s64 subscr)
{
	u64 blocknum, u64_within_block;
	u64 *blockp;
	int shift = 0;
	...
	return blockp[u64_within_block] >> shift;
}
...
//#endif
	...
	/* First word is rdtsc with CPU# placed in top two bytes */
...
static u64 insert_n_user(u64 word)
{
	...
	temp[0] = 0;
	uncopied_bytes = copy_from_user(temp, userptr, 8 * sizeof(u64));
	if (uncopied_bytes > 0)
		return 0;
...
{
//	u32 scno = event & 0x01ffllu;
//	if (event & KUTRACE_SYSCALL64 && scno != 78 && scno != 3 && scno != 409)
//		printk(KERN_INFO "Syscall %u received with arg %u\n", (u32)(event & 0x01ffllu), (u32)arg);
...
static int __init kutrace_mod_init(void)
{
    printk(KERN_INFO "get_cycles call #0: %ld\n", get_cycles());
    printk(KERN_INFO "get_cycles call #1: %ld\n", get_cycles());
    printk(KERN_INFO "get_cycles call #2: %ld\n", get_cycles());
    printk(KERN_INFO "get_cycles call #3: %ld\n", get_cycles());
    printk(KERN_INFO "get_cycles call #4: %ld\n", get_cycles());
    printk(KERN_INFO "get_cycles call #5: %ld\n", get_cycles());
    printk(KERN_INFO "get_cycles call #6: %ld\n", get_cycles());
    printk(KERN_INFO "get_cycles call #7: %ld\n", get_cycles());
    printk(KERN_INFO "get_cycles call #8: %ld\n", get_cycles());
    printk(KERN_INFO "get_cycles call #9: %ld\n", get_cycles());
	printk(KERN_INFO "\nkutrace_trace hello =====================\n");
	printk(KERN_INFO "kutrace_control syscall is: %u\n", __NR_kutrace_control);
	kutrace_tracing = false;
	...
	volatile u32 * TIMER_CTRL_BASE = ioremap(0x00A00208, 4);
	*TIMER_CTRL_BASE |= 0x00000001;
	// Wait for user-mode cycle counter access to be enabled for each CPU
	on_each_cpu(enable_usermode_cyclecount, NULL, 1);
#endif
